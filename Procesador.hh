...
/** @brief Constructora por defecto
    \pre <em>cierto</em>
    \post El resultado es un procesador con un id y una memoria no inicializadas
*/
    Procesador();

/** @brief Constructora con valores concretos
    \pre id solo contiene letras y numeros, y el tamaño de la memoria
    \post El resultado es un procesador con el id y la memoria pasados por parámetro
*/
...
/** @brief Elimina un proceso del parámetro implícito
    \pre <em>cierto</em>
    \post El proceso p se elimina del parámetro implícito, dejando libre la memoria usada por este proceso
    La función devuelve 'true' si no ha habido ningún error, en caso contrario devuelve 'false'
*/
    bool eliminar_proceso(const Proceso& p);
...
*/
    bool existe_proceso(const Proceso& p) const;
...
/** @brief Consultora de cuan ajustado es el espacio de memoria respecto un proceso
    ...
    \post Devuelve el tamaño que tiene el hueco de memoria más pequeño que es capaz de contener el proceso p pasado por parámetro
    Si no existe tal hueco, la función devuelve -1
*/
    int hueco_mas_ajustado(const Proceso& p) const;
